...
export function AppShell() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-display">
      <nav className="sticky top-0 z-50 flex items-center justify-between px-6 py-3 border-b border-border bg-background/80 backdrop-blur-md">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-border">
            ...
            <span className="font-accent text-lg text-primary leading-none">${ME.balance.toFixed(2)}</span>
          </div>
        </div>
      </nav>
      ...
      <footer className="fixed bottom-0 w-full bg-black/80 backdrop-blur-xl border-t border-border px-6 py-2 flex items-center justify-between z-40">
        <div className="flex gap-6 overflow-hidden text-xs">
          <span className="font-mono"><span className="text-white/40">BTC </span>$64,120.50</span>
          <span className="font-mono hidden sm:inline"><span className="text-white/40">GAS </span><span className="text-success">12 GWEI</span></span>
          <span className="font-mono hidden md:inline"><span className="text-white/40">STAKED </span><span className="text-primary">$4,204,192</span></span>
        </div>
